...
void ConsoleScreen::start() {
    ...
    // Draw header
    drawHeader("SYSTEM CONSOLE", true);
    drawKeyboard();

    // Welcome message
    clearScreen();
    printLine("Emerald System Console v1.0", COLOR_NEON2);
    printLine("Type 'help' for commands.", COLOR_DIM);
...
void ConsoleScreen::executeCommand(const char* cmd) {
    ...
    if (strcmp(cmd, "help") == 0) {
        printLine("Commands:", COLOR_NEON);
        printLine("  help     - Show this help", COLOR_TEXT);
        printLine("  whoami   - Show user info", COLOR_TEXT);
        printLine("  date     - Show date/time", COLOR_TEXT);
        printLine("  status   - System status", COLOR_TEXT);
        printLine("  scan     - WiFi scan", COLOR_TEXT);
        printLine("  sensors  - Fake sensor data", COLOR_TEXT);
        printLine("  about    - About Emerald", COLOR_TEXT);
        printLine("  clear    - Clear screen", COLOR_TEXT);
        printLine("  fedir    - Secret message", COLOR_TEXT);
        ...
        printLine("System time: 09:41", COLOR_TEXT);
        printLine("System date: 2026-05-28", COLOR_TEXT);
    } else if (strcmp(cmd, "status") == 0) {
        printLine("System: ONLINE", COLOR_NEON);
        printLine("CPU: 240MHz", COLOR_TEXT);
        printLine("Achievements: 21/21", COLOR_TEXT);
        printLine("Devices connected: 0", COLOR_TEXT);
    } else if (strcmp(cmd, "scan") == 0) {
        printLine("Starting WiFi scan...", COLOR_DIM);
        printLine("Use Wi-Fi screen for details.", COLOR_TEXT);
    } else if (strcmp(cmd, "sensors") == 0) {
        printLine("CPU temp: 42.5 C", COLOR_TEXT);
        printLine("System voltage: 3.3V", COLOR_TEXT);
        printLine("WiFi signal: -54 dBm", COLOR_TEXT);
    } else if (strcmp(cmd, "about") == 0) {
        char buf[64];
        snprintf(buf, sizeof(buf), "%s %s", OS_NAME, OS_VERSION);
        printLine(buf, COLOR_NEON);
        printLine("A personal gadget for young programmers.", COLOR_TEXT);
        printLine("Created for Fedir.", COLOR_TEXT);
    } else if (strcmp(cmd, "clear") == 0) {
        clearScreen();
    } else if (strcmp(cmd, "fedir") == 0) {
        printLine("The future belongs to you, Fedir.", COLOR_NEON);
