...
void ProfileScreen::drawContent() {
    ...
    char buf[40];
    snprintf(buf, sizeof(buf), "Level %d  %s", USER_LEVEL, USER_TITLE);
    tft.drawString(buf, 68, 54, 1);
    tft.drawString("robocode.ua  Intro Coding", 68, 66, 1);
    ...
    drawStatBox(8,   94, 70, 40, "Achievements", "21");
    drawStatBox(82,  94, 70, 40, "Technologies", "12+");
    drawStatBox(156, 94, 70, 40, "Projects",     "8");
    drawStatBox(8,   142, 70, 40, "Kahoot Wins",  ":)");

    // Badges label
    tft.setTextDatum(TL_DATUM);
    tft.setTextSize(1);
    tft.setTextColor(COLOR_DIM, COLOR_BG);
    tft.drawString("BADGES:", 8, 192, 1);
