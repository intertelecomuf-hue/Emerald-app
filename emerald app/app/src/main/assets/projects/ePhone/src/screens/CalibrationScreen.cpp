...
void CalibrationScreen::start() {
    ...
    tft.drawString("KALIBRACIYA TOUCH", DISP_WIDTH / 2, 10, 2);
    ...
    // Status bar
    drawStatusBarType("Tapni komirku 1 dlya pochatku", COLOR_YELLOW);
...
void CalibrationScreen::finalizePoint() {
    ...
    // Update status
    char status[32];
    snprintf(status, sizeof(status), "Trymay komirku %d", _currentPoint + 1);
...
void CalibrationScreen::handleTap(int x, int y) {
    ...
    if (_currentPoint == -1) {
        ...
        _sampleSumY[0] = 0;
        _sampleCount[0] = 0;
        drawStatusBarType("Trymay komirku 1", COLOR_NEON);
...
void CalibrationScreen::drawTestScreen() {
    ...
    tft.setTextSize(1);
    tft.setTextDatum(CC_DATUM);
    tft.drawString("TEST KALIBRACIYi", DISP_WIDTH / 2, 15, 2);
    
    tft.setTextColor(COLOR_TEXT, COLOR_BLACK);
    tft.drawString("Tapny bud-de dlya testu mapuvannya", DISP_WIDTH / 2, 40, 1);
    tft.drawString("Chervona tochka = vidobrazhennya", DISP_WIDTH / 2, 55, 1);
    ...
    tft.setTextDatum(CC_DATUM);
    tft.drawString("Dovgyy tap dlya vyhodu v menu", DISP_WIDTH / 2, DISP_HEIGHT - 15, 1);
    tft.setTextDatum(TL_DATUM);
    
    drawStatusBarType("REZhYM TESTU", COLOR_YELLOW);
}
